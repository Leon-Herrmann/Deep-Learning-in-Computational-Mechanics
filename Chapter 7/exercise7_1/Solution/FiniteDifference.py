...
def solve_waveequation1D(I, V, f, c, x, Nx, dt, N, bc='Neumann'):
    """Solve the 1D wave equation with finite differences."""
    ...
    if bc=='Dirichlet':
        ...
        u[1:-1] = u_1[1:-1] + dt*V(x[1:-1]) + \
                  ...
                  0.5*dt**2*f[n, 1:-1]
        # Insert homogeneous boundary conditions
        u[0] = 0
        u[Nx] = 0
    ...
    # Neumann boundary conditions
        ...
        u[1:-1] = u_1[1:-1] + dt*V(x[1:-1]) + \
                  0.5*C2[1:-1]*(u_1[:-2] - 2*u_1[1:-1] + u_1[2:]) + \
                  0.5*dt**2*f[n, 1:-1]
        # Insert boundary conditions with special formula
        u[0] = u_1[0] + dt*V(x[0]) + \
